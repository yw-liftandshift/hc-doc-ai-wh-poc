locals {
  cloud_function_dir = "${path.module}/../../../../../../postprocess-ocr-cloud-function"

  cloud_function_zip_path = "${path.module}/postprocess-ocr-cloud-function.zip"
}
...
resource "google_storage_bucket_object" "cloud_function_code" {
  name   = "postprocess-ocr-cloud-function.${data.archive_file.cloud_function_code.output_md5}.zip"
  bucket = google_storage_bucket.cloud_function_code.name
  source = local.cloud_function_zip_path
}

resource "google_cloudfunctions2_function" "postprocess_ocr" {
  name        = "postprocess-ocr"
  location    = "northamerica-northeast1"
  description = "Postprocess OCR Batch Result"

  build_config {
    runtime     = "python312"
    entry_point = "main"

    docker_repository = google_artifact_registry_repository.postprocess_ocr_cloud_function.id
  ...
  service_config {
    service_account_email = var.postprocess_ocr_cloud_function_sa_email
