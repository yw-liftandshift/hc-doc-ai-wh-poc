locals {
  log_buccket_id = "recognition_result_log"
  filter = "projects/${data.google_project.project.project_id}/logs/${local.log_buccket_id}"
}

resource "google_logging_project_sink" "recognition_result_log_sink" {
  name = "${local.log_buccket_id}_sink"

  destination = "logging.googleapis.com/projects/${data.google_project.project.project_id}/locations/global/buckets/${local.log_buccket_id}"
